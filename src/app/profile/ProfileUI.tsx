...
export default function ProfileUI() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 space-y-10">
      <h1 className="text-2xl font-bold text-gray-800 text-center lg:text-left">Профиль пользователя</h1>
      {message && <p className="text-green-600 text-center lg:text-left">{message}</p>}

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-shrink-0 w-full sm:w-48 h-48 relative group rounded-lg overflow-hidden">
            <img
            ...
            <label
              ...
              className="absolute inset-0 bg-black bg-opacity-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition"
            >
      ...
      <form onSubmit={handlePasswordSubmit} className="space-y-4 max-w-md mx-auto">
        <h2 className="text-xl font-semibold text-gray-800 text-center lg:text-left">Изменить пароль</h2>
        {pwdMessage && <p className="text-red-600 text-center lg:text-left">{pwdMessage}</p>}
